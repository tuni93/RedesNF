...
%% Agrego la entrada con 1 del perceptron
x = [x, ones(2^n_entradas, 1)];
%% Salida deseada del sistema
yd = [  -1;
         1;
         1;
        -1;
...
%% Generacion de la poblacion de arranque
Poblacion = [];
...
while(ECM > MAX_ERROR)
    ...
    for i = 1 : NUMERO_DE_INDIVIDUOS
        ...
        Fitness_individuos(i) = funcionFit(individuo, W, w, x, yd, n_entradas, beta);
    end
    
    %% Ordeno la poblacion segun el fit
    [Fitness_individuos, I] = sort(Fitness_individuos, 'descend');
    Poblacion = Poblacion(:, I);
    
    %% Recombinacion de las peores 90 combinaciones
    Nueva_Poblacion = Poblacion(:, 1:10);
    for j = 1 : 90
        primer_elemento = floor(rand * (NUMERO_DE_INDIVIDUOS - 1)) + 1;
        segundo_elemento = floor(rand * (NUMERO_DE_INDIVIDUOS - 1)) + 1;
        corte = floor(rand * (size(Poblacion, 1) - 1)) + 1;
        primera_parte =Poblacion(1 : corte, primer_elemento);
        segunda_parte = Poblacion(corte + 1 : size(Poblacion, 1), segundo_elemento);
        nuevo_individuo = [primera_parte; segunda_parte];
        Nueva_Poblacion = [Nueva_Poblacion, nuevo_individuo];
    end
    Poblacion = Nueva_Poblacion;
  
    %Mutaciones
    for j = 1 : size(Poblacion, 1)
        for k = 1 : size(Poblacion, 2)
            p = rand;
            if(p < probabilidad_de_mutar)
                Poblacion(j, k) = Poblacion(j, k) + randn * desvio_de_mutacion;
            end
        end
    end
    %Recalculo fitness.
    ...
    %Eligiendo el de mejor fit, es el primero.
    ECM = 1 / funcionFit(Poblacion(:, 1), W, w, x, yd, n_entradas, beta);
end

%Mejor individuo.
W = vec2mat(Poblacion(1 : 6, 1), n_entradas + 1);
w = vec2mat(Poblacion(7 : 9, 1), neuronas_capa_1 + 1);
